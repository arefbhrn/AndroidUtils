...
class LubanCompresser {
    ...
    Observable<File> singleAction(final File file) {
        return Observable.fromCallable(() -> compressImage(mLuban.gear, file))
    ...
    Observable<List<File>> multiAction(List<File> files) {
        ...
        for (final File file : files) {
            observables.add(Observable.fromCallable(() -> compressImage(mLuban.gear, file)));
        }

        return Observable.zip(observables,
                        args -> {
                            List<File> files1 = new ArrayList<>(args.length);
                            for (Object o : args) {
                                files1.add((File) o);
                            }
                            return files1;
    ...
    private File firstCompress(@NonNull File file) throws IOException {
        ...
        if (imgSize[0] <= imgSize[1]) {
            ...
            if (scale <= 1.0 && scale > 0.5625) {
                width = Math.min(imgSize[0], shortSide);
                height = width * imgSize[1] / imgSize[0];
                size = minSize;
            } else if (scale <= 0.5625) {
                height = Math.min(imgSize[1], longSide);
            ...
            if (scale <= 1.0 && scale > 0.5625) {
                height = Math.min(imgSize[1], shortSide);
                width = height * imgSize[0] / imgSize[1];
                size = minSize;
            } else if (scale <= 0.5625) {
                width = Math.min(imgSize[0], longSide);
                height = width * imgSize[1] / imgSize[0];
                size = maxSize;
            }
        }
    ...
    private String getCacheFilePath() {
        ...
        Bitmap.CompressFormat format = mLuban.compressFormat;
        boolean isWEBP = format == Bitmap.CompressFormat.WEBP ||
                (isOsAtLeast(Build.VERSION_CODES.R) &&
                        (format == Bitmap.CompressFormat.WEBP_LOSSLESS || format == Bitmap.CompressFormat.WEBP_LOSSY));
        if (isWEBP) {
            name.append(".webp");
        } else {
            name.append(".jpg");
